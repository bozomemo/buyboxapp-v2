...
describe('resolveExportColumns', () => {
  ...
  it('drops the per-row competitive columns', () => {
    ...
    expect(resolveExportColumns(['label', 'sellerCount', 'buyboxPrice', 'buyboxSeller'])).toEqual([
      'label',
    ]);
  });
...
describe('exportHeaders / exportRow', () => {
  ...
  it('labels how each product was discovered', () => {
    ...
    expect(exportRow(columns, { ...ROW, viaBrandRef: false, viaSearchTerm: true })).toEqual([
      'sadece arama',
    ]);
    expect(
