...
export async function GET() {
  ...
  const [marketplaces, globalKillSwitch, phaseDistribution, alerts, decisions, lastImport, lastBuybox] =
    await Promise.all([
      configRepo.listMarketplaces(appDb),
      configRepo.getAppSetting(appDb, 'global.killSwitch'),
  ...
  return NextResponse.json({
    globalKillSwitchEngaged: globalKillSwitch?.value === 'true',
